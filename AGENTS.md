...
When building the game, never do more than one development roadmap step at a time. Each step must be playable and will be validated by a human game tester before proceding. godot is available on the bash command line. You can run it to see errors in your godot scripts so you can fix them.
